...
let
  ...
  recDeps =
  let
    isPackage = pkg: lib.isDerivation pkg && lib.any (x: x == "recipeFile") (lib.attrNames pkg);
...
let
  deriv = stdenv.mkDerivation rec {
    ...
    emacsPackages = toPaths (recDeps (topPackages extractedPackages));
    ...
    emacsD = byteCompileDir emacsDir siteStart;
    ...
    installPhase = ''
     ...
     # Link in the byte-compiled emacs.d
     echo $emacsD
     ln -s $emacsD $out/emacs.d
    '';
    inherit (emacs) meta;
  };
